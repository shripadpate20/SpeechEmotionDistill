...
# # Unfreeze only projector and classifier
# for name, param in model.named_parameters():
#     if name.startswith("projector") or name.startswith("classifier"):
...
for epoch in range(EPOCHS):
    ...
    for waveforms, labels in tqdm(train_loader, desc=f"Epoch {epoch+1}/{EPOCHS} - Training"):
        ...
        waveforms = waveforms.to(DEVICE)  # Adds channel dimension
        ...
        all_preds.extend(torch.argmax(outputs.logits, dim=1).cpu().numpy())  # <- FIXED
    ...
    with torch.no_grad():
        for waveforms, labels in tqdm(val_loader, desc=f"Epoch {epoch+1}/{EPOCHS} - Validation"):
            ...
            val_preds.extend(torch.argmax(outputs.logits, dim=1).cpu().numpy())  # <- FIXED
...
# Save the model
torch.save(model.state_dict(), "crema_cnn_model.pth")
